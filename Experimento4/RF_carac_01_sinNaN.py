...
columnasDeseadas = ['PeakTime', 'PeakAmp', 'MinF0', 'MeanF0', 'Jitter', 'Shimmer', 'StDevHNR'] # 0.1 caracterisiticas

# Obtenemos X
emociones_a_filtrar = ['happy', 'sad', 'angry']
X_Filtrada = data[data['Emotion_Label'].isin(emociones_a_filtrar)]
X = X_Filtrada[columnasDeseadas]

# Obtenemos Y
emociones_filtradas = data[data['Emotion_Label'].isin(emociones_a_filtrar)]  # columna objetivo
y = emociones_filtradas['Emotion_Label']
...
for intento in range(numIntentos):
    ...
    # Generamos y entrenamos el modelo Random Forest en teoria
    rf_classifier = RandomForestClassifier(n_estimators=100, random_state=intento)
    ...
    sns.heatmap(conf_matrix, annot=True, fmt='d', cmap='Blues', xticklabels=['angry', 'happy', 'sad'], yticklabels=['angry', 'happy', 'sad'])
...
# Visualizar la matriz de confusión con seaborn
plt.figure(figsize=(6, 4))
sns.heatmap(matrix_confusion_total_enteros, annot=True, fmt='d', cmap='Blues', xticklabels=['angry', 'happy', 'sad'], yticklabels=['angry', 'happy', 'sad'])
plt.xlabel('Predicciones')
plt.ylabel('Valores Reales')
